...
# [WIP] 按“图”索骥：从 vLLM Ascend 的图模式聊开去

## 太长不看

本文大致分四段：第一章先在 vLLM 官方图模式文档的视角下，解释在 Ascend 硬件上额外做了哪些工程补偿；第二章转去讲 `torch.compile`、Inductor、TorchAir 和 `npugraph_ex` 这些编译路径；第三章回到具体设计，拆开图模式里的动态性、分桶、不同 graph mode 与性能权衡；最后两章再把视角抬高，回头看这些技术细节背后的生态问题。
